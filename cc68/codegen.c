...
/* Generate the assembler code preamble */
{
    ...
    AddTextLine ("\t.fopt\t\tcompiler,\"cc68 v %s\"",
                 GetVersionAsString ());
   ...
   cases we want to optimize */
{
    ...
    if ((Flags & CF_CONST) != 0) {
        ...
        switch (Flags & CF_TYPEMASK) {
            ...
            case CF_CHAR:
                if ((Flags & CF_INTOX) == 0 && (Flags & CF_FORCECHAR) != 0) {
                    if (Val == 0)
                        AddCodeLine("clrb");
                    else
...
/* Fetch the address of the specified symbol into the primary register */
{
    ...
    /* When we support via X variants we can be smarter but for now .. */
...
/* Push the primary register or a constant value onto the stack */
{
    if ((flags & CF_CONST) && (flags & CF_TYPEMASK) != CF_LONG) {
        ...
        if ((flags & CF_TYPEMASK) == CF_CHAR && (flags & CF_FORCECHAR)) {
            ...
            g_getimmed (CF_INTOX|flags, val, 0);
            AddCodeLine ("pshx");
        }
        ...
        if (flags & CF_CONST) {
            ...
            if ((flags & CF_TYPEMASK) != CF_CHAR || (flags & CF_FORCECHAR) == 0) {
                g_getimmed (flags | CF_INTOX, val, 0);
                switch(flags & CF_TYPEMASK) {
                case CF_INT:
                    AddCodeLine("pshx");
                    break;
